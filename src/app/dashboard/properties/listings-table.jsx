...
const StatusPill = ({ status }) => {
  ...
  return (
    ...
    >
      {status === "processing" ? "Pending" : status?.charAt(0).toUpperCase() + status?.slice(1)}
    </span>
  )
}
...
const getFilteredListings = async (page = 1, limit = 10, status = 'all') => {
  try {
    const response = await axios.get(`${API_URL}/properties/admin/filtered-listings`, {
...
const approveListing = async (listingId) => {
  try {
    const response = await axios.put(`${API_URL}/properties/admin/approve/${listingId}`)
    return response.data
  } catch (error) {
...
export function ListingsTable() {
  ...
  const fetchFilteredListings = useCallback(async (status = statusFilter) => {
    setLoading(true)
    try {
  ...
  const handleConfirmDelete = useCallback(async () => {
    if (listingToDelete) {
      try {
        const response = await fetch(`${API_URL}/properties/user-property/${listingToDelete._id}`, {
          method: "DELETE",
        })
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => handleApproveListing(listing._id)}>
                  Approve
                </DropdownMenuItem>
